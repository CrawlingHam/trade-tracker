...
function YearlySummary({ items, formatSignedMoney }: Pages.Dashboard.Props.YearlySummary): JSX.Element {
	return (
		<div className="overflow-x-auto">
			<p className="mb-2 text-[10px] uppercase tracking-[0.08em] text-(--text3)">Yearly (realized)</p>
			...
			<table className="w-full border-collapse text-xs">
				...
				<tbody>
					{items.map(
						(row): JSX.Element => (
							<tr key={`y-${row.label}`} className="border-b border-(--border)">
